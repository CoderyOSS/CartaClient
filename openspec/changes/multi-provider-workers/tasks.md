...
- [ ] 1.1 Add `ProviderKind` enum with `Docker`, `Daytona`, `MicroK8s`, `Localhost` variants
...
- [ ] 2.3 Add `DaytonaProviderConfig`, `MicroK8sProviderConfig`, `LocalhostProviderConfig` structs
- [ ] 2.4 Add accessor methods: `daytona_provider()`, `microk8s_provider()`, `localhost_provider()`
...
- [ ] 5.2 Implement sandbox creation via `POST /sandbox` with `autoStopInterval: 0`, `autoDeleteInterval: 0`
- [ ] 5.3 Implement status polling: `GET /sandbox/{id}` until state == "started" or timeout
- [ ] 5.4 Implement worker destruction: `DELETE /sandbox/{id}`
- [ ] 5.5 Implement log retrieval: `GET /sandbox/{id}/logs?tail=N`
- [ ] 5.6 Implement `list_workers`: `GET /sandbox` with label selector `trailhead=true`
- [ ] 5.7 Project content delivery: git clone inside sandbox
- [ ] 5.8 Spawn `refresh_activity` heartbeat task for stage lifetime (if `autoStopInterval > 0`)
- [ ] 5.9 `cargo test -p trailhead-service` passes

## 6. MicroK8s Provider (`provider/microk8s.rs`)
...
- [ ] 6.9 Default kubeconfig path: `/var/snap/microk8s/current/credentials/client.config`
- [ ] 6.10 Document MicroK8s setup: `microk8s enable dns storage registry`
- [ ] 6.11 `cargo test -p trailhead-service` passes
...
- [ ] 9.3 `cargo check -p trailhead-service` with `--no-default-features` succeeds (microk8s feature-gated)
